...
device_info = {}  # معلومات الأجهزة {device_id: {name, last_seen}}
...
print("=" * 60)
print("🎯 سيرفر التحكم المتكامل - الإصدار النهائي")
print("=" * 60)
print(f"📡 المنفذ: {PORT}")
print("=" * 60)
    ...
    try:
        async for message in websocket:
            try:
                ...
                log(f"📩 {msg_type} من {device_id}", "RECEIVE")
                ...
                # ===== أمر غير معروف =====
                    ...
                    log(f"⚠️ أمر غير معروف: {msg_type}", "WARNING")
                    await websocket.send(json.dumps({
                        "type": "ERROR",
                ...
                log(f"❌ رسالة غير صالحة", "ERROR")
            except Exception as e:
                log(f"❌ خطأ في المعالجة: {e}", "ERROR")
